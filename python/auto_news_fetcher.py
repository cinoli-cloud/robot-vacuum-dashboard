...
class MultiSourceNewsFetcher:
    ...
    def extract_real_url_from_google(self, google_url):
        ...
        Google News URL格式：https://news.google.com/rss/articles/CBMi...
        """
        import base64
        import re
        ...
        try:
            ...
            if match:
                ...
                try:
                    ...
                    # 查找http链接
                    url_match = re.search(r'https?://[^\s\x00\x01\x02\x03]+', decoded_str)
                    if url_match:
                        real_url = url_match.group(0)
                        # 清理URL末尾的特殊字符
                        real_url = re.sub(r'[\x00-\x1f]+.*$', '', real_url)
                        logger.info(f"Extracted real URL: {real_url[:80]}...")
                        return real_url
                except Exception as decode_error:
                    logger.debug(f"Failed to decode Google News URL: {decode_error}")
        except Exception as e:
            logger.debug(f"Error extracting real URL: {e}")
