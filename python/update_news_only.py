...
def update_news_in_json():
    ...
    # 验证URL并打印示例（调试用）
    print(f"\n🔍 URL验证（前5条示例）：")
    google_redirect_count = 0
    real_url_count = 0

    for idx, news in enumerate(new_news[:5], 1):
        url = news.get('url', 'NO_URL')
        is_google_redirect = 'news.google.com/rss/articles/' in url

        if is_google_redirect:
            google_redirect_count += 1
            url_type = "❌ Google重定向（未解析）"
        else:
            real_url_count += 1
            url_type = "✅ 真实链接"

        print(f"  [{idx}] {news.get('title', '')[:50]}...")
        print(f"      状态: {url_type}")
        print(f"      URL: {url[:80]}...")
    ...
    # 统计总体URL质量
    total_google = sum(1 for n in new_news if 'news.google.com/rss/articles/' in n.get('url', ''))
    total_real = len(new_news) - total_google
    print(f"📊 URL质量统计：")
    print(f"  ✅ 真实链接: {total_real} 条 ({total_real/len(new_news)*100:.1f}%)")
    print(f"  ❌ Google重定向: {total_google} 条 ({total_google/len(new_news)*100:.1f}%)")
    print()
