...
function renderPriceTable(products) {
    ...
    filteredProducts.forEach(product => {
        ...
        // Channels (移除Amazon列，24h Change整合到价格单元格内)
        const officialCell = formatPriceCell(product.channels.official);
        const bestbuyCell = formatPriceCell(product.channels.bestbuy);
        ...
        row.innerHTML = brandCell + productCell + msrpCell + officialCell + bestbuyCell + walmartCell + costcoCell + ebayCell;
...
function formatPriceCell(channel) {
    ...
    // Add 24h change indicator (only if there's a change)
    let changeTag = '';
    if (channel.change_24h && Math.abs(channel.change_24h) >= 0.1) {
        const changeValue = channel.change_24h.toFixed(1);
        if (channel.change_24h > 0) {
            changeTag = ` <span class="change-indicator change-up" style="color: #dc3545; font-size: 0.85em;">↑${changeValue}%</span>`;
        } else {
            changeTag = ` <span class="change-indicator change-down" style="color: #198754; font-size: 0.85em;">↓${Math.abs(changeValue)}%</span>`;
        }
    }

    // Add clickable link if URL is available
    if (channel.url) {
        return `<td class="${priceClass}"><a href="${channel.url}" target="_blank" rel="noopener noreferrer" class="price-link" title="Click to verify actual price">${price}</a>${confidenceTag}${changeTag}</td>`;
    }

    return `<td class="${priceClass}">${price}${confidenceTag}${changeTag}</td>`;
}
